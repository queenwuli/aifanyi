module.exports = (Ferdium, settings) => {
  ...
  const classnameCfg = {
    main: '.conversation__inner',
    allMsg: '.conversation__inner .bubble__text__inner.ng-binding',
    sendBtn: 'button.md-icon-button.editor__item.editor__send.md-button',
    groupflagEl: '.conversation'
  };
  ...
  const handleSendMessage = async (documents, context) => {
    ...
    let textareaEl = documents.parentNode.querySelector('textarea')
    if (res.body.code === 200 && res.body.data) {
      ...
      textareaEl.dispatchEvent(evtInput);
      setTimeout(() => {
        document.querySelector(classnameCfg.sendBtn)?.click();
      }, 500);
    }
  ...
  const getMainView = () => {
    let textarea = document.querySelector('.editor__typearea')
    if(textarea && !textarea.className.includes('ng-hide')) {
  ...
  const isGroup = () => {
    ...
    return el?.className.includes('is-group')
  }
  ...
  const freshChatList = (e) => {
    ...
    for (const msg of msgList) {
      ...
      const isGroupFalg = msg.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.className.includes(
        'in-group',
      );
      if (check && text) {
        if ((oneworld.settingCfg.sendtranslation && !isOwn) || (oneworld.settingCfg.tranflag && isOwn)) {
          // 如果是群聊则跟进群聊开关判断
          if((isGroupFalg && oneworld.settingCfg.groupflag) || !isGroupFalg) {
