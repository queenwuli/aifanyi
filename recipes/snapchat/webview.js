...
module.exports = (Ferdium, settings) => {
  ...
  function getIptSendMsg() {
   ...
   function showLoading(txt, type){
        ...
        if(el) {
            ...
            let main = document.querySelector('.IBqK8')
            if(main) {
                main.style.position='relative'
                main.style.overflowX='hidden'
                main.insertAdjacentHTML(
                    'beforeend',
                    `<span id="aitansLoading" style="font-size:14px;position:absolute;z-index:999;left:0;right: 0;margin: 0 auto;border-radius: 4px;text-align:center;bottom:10px;color:${color};">${txt}</span>`
                );
            };
        }
    }
  ...
  async function showChineseTrans(msg) {
    ...
    if(el) {
      ...
      if(main) {
        main.insertAdjacentHTML(
            'beforeend',
            `<div id="aitansChinese" style="font-size:14px;padding: 5px 10px;background: #fff;">${txt}</div>`
        );
      };
    }
  }
  ...
  // 判断是群聊还是私聊, true 群聊
  function isGroup() {
    let els = document.querySelectorAll(classnameCfg.groupflagEl)
    if(els.length > 1) {
      return true
    }
    return false
  ...
  function freshChatList() {
    ...
    for (const msgDiv of msgList1) {
      ...
      let check = true
      if(msgDiv.nextSibling && msgDiv.nextSibling.className && (msgDiv.nextSibling.className.includes('autofanyi') || msgDiv.nextSibling.className.includes('click-fanyi'))) {
        check = false
      }
      if(check) {
        let isOwn = msgDiv.parentNode.parentNode.parentNode.parentNode.querySelector('.SGcAb')?.textContent?.trim() === '我' || msgDiv.parentNode.parentNode.parentNode.parentNode.parentNode.querySelector('.R1ne3')?.textContent?.trim() === '我';
        console.log(isOwn, msg)
  ...
  async function autoFanyi(msg, msgDiv, isOwn) {
    let autoFanyi = msgDiv.nextSibling && msgDiv.nextSibling.className.includes('autofanyi') ? msgDiv.nextSibling : null;
    if(!autoFanyi) {
      return
    }
    if(!msg || isNumber(msg)){
