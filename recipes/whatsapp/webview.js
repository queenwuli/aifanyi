...
module.exports = (Ferdium, settings) => {
  ...
  const getChatId = () => {
    ...
    return 'whatsapp-'+el.textContent
  }
    ...
    document.addEventListener(
      ...
      (e) => {
        if(getFriendView()?.contains(e.target)) {
          ...
          Ferdium.ipcRenderer.send('setChatInfo', chatId, 'whatsapp')
        }
        setTimeout(() => {
      ...
      (function(j){
          moreELs[j].onclick = function () {
            ...
            setTimeout(() => {
				...
				const text = div.querySelector('._11JPr.selectable-text.copyable-text') && div.querySelector('._11JPr.selectable-text.copyable-text').textContent;
